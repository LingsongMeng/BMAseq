Bayesfactor.Laplace <- function(X, y, w) {
    ...
    ### this function implement the Laplace approximation of the Bayes factor in Liang et. al. (2008), which use Zellner-Siow prior.  The Bayes factor calculated is based
    ### on the Null model.
    ...
    ## find the mode
    myfunction <- function(g) -(1 - rsquared) * (p + 3) * g^3 + (n - p - 4 - 2 * (1 - rsquared)) * g^2 + (n * (2 - rsquared) - 3) * g + n
    myroot = uniroot(myfunction, interval = c(0, 1e+10))
    ghat = myroot$root
    
    ## define the function h(g)
    h <- function(g) (n - p - 1)/2 * log(1 + g) - (n - 1)/2 * log(1 + (1 - rsquared) * g) + 0.5 * log(n/2) - log(sqrt(pi)) - 1.5 * log(g) - n/2/g
